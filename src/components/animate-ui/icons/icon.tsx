...
function AnimateIcon({
  ...
  React.useEffect(() => {
    ...
    const anim = typeof animate === 'string' ? animate : animation;
    queueMicrotask(() => {
  ...
  const startAnim = React.useCallback(
    async (anim: 'initial' | 'animate', method: 'start' | 'set' = 'start') => {
      try {
        await (controls as unknown as Record<string, (a: string) => Promise<unknown>>)[method](anim);
        setStatus(anim);
      } catch {
        return;
      }
  ...
  }, [localAnimate, controls, completeOnStop, persistOnAnimateEnd, loop, loopDelay, initialOnAnimateEnd, startAnim, status]);
  ...
  const handlePointerDown = React.useCallback(
    (e: React.PointerEvent<HTMLElement>) => {
      (childProps.onPointerDown as ((e: React.PointerEvent<HTMLElement>) => void) | undefined)?.(e);
  ...
  const handlePointerUp = React.useCallback(
    (e: React.PointerEvent<HTMLElement>) => {
      (childProps.onPointerUp as ((e: React.PointerEvent<HTMLElement>) => void) | undefined)?.(e);
...
function IconWrapper<T extends string>({
  ...
  const finalAnimate = isHovered !== undefined ? (isHovered ? (animationProp ?? 'default') : false) : animate;
  const finalAnimateOnHover = animateOnHover ?? (finalAnimate === undefined && animateOnTap === undefined && animateOnView === undefined ? true : undefined);

  return (
    <AnimateIcon
      animate={finalAnimate}
      animateOnHover={finalAnimate !== undefined ? undefined : finalAnimateOnHover}
      animateOnTap={animateOnTap}
      animateOnView={animateOnView}
      animateOnViewMargin={animateOnViewMargin}
      animateOnViewOnce={animateOnViewOnce}
