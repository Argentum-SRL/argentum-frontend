...
export default function PhoneLoginPage() {
  ...
  const telefonoError =
    hasSubmitted && !telefono.trim() ? 'Ingresá tu número de teléfono.' : null
  ...
  async function handleVerifyCode(e: FormEvent) {
    ...
    setApiError(null)
    try {
      const data = await verificarCodigoTelefono(telefono.trim(), codigo.trim())
      ...
      const detail = err.response?.data?.detail
      if (status === 400) {
        setApiError(detail || 'Código incorrecto o expirado.')
      } else {
      ...
      <AuthLayout title="Ingresá con tu teléfono">
        <form onSubmit={handleSendCode} noValidate>
          ...
          <div className="mb-6">
            ...
            {telefonoError && (
              <p className="mt-1" style={{ fontSize: '13px', color: '#C0392B' }}>
                {telefonoError}
              </p>
            )}
      ...
      <form onSubmit={handleVerifyCode} noValidate>
        ...
        <div className="mb-6">
          ...
          <input
            ...
            placeholder="000000"
            className="w-full h-14 px-4 rounded-[10px] border outline-none transition-colors text-center tracking-[0.5em]"
            style={{
              fontSize: '24px',
              fontWeight: 600,
              letterSpacing: '0.5em',
